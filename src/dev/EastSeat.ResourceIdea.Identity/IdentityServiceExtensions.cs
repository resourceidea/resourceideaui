...
using EastSeat.ResourceIdea.Identity.Services;
...
public static class IdentityServiceExtensions
{
    public static void AddIdentityServices(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        services.AddIdentity<ApplicationUser, IdentityRole>()
                .AddEntityFrameworkStores<ResourceIdeaIdentityDbContext>().AddDefaultTokenProviders();

        services.AddDbContext<ResourceIdeaIdentityDbContext>(
            options => options.UseSqlServer(configuration.GetConnectionString("IdentityConnectionString"),
            b => b.MigrationsAssembly(typeof(ResourceIdeaIdentityDbContext).Assembly.FullName))
        );
