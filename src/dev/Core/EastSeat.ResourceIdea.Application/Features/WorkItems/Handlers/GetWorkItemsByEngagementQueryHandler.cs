...
public class GetWorkItemsByEngagementQueryHandler(IWorkItemsService workItemsService)
    : BaseHandler,
      IRequestHandler<GetWorkItemsByEngagementQuery, ResourceIdeaResponse<PagedListResponse<WorkItemModel>>>
{
    private readonly IWorkItemsService _workItemsService = workItemsService;

    public async Task<ResourceIdeaResponse<PagedListResponse<WorkItemModel>>> Handle(GetWorkItemsByEngagementQuery request, CancellationToken cancellationToken)
    {
        WorkItemsByEngagementSpecification specification = new(request.EngagementId, request.TenantId);
